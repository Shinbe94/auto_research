...
from src.pages.savior.ngoisao_page import NgoisaoPage
from tests.savior.savior_base_test import SaviorBaseTest
from src.utilities import os_utils, file_utils, media_utils


class TestNgoisao(SaviorBaseTest):
    def test_ngoisao(self):
        self.ngoisao_page = NgoisaoPage(self)
        file_name = None
        try:
            data = self.ngoisao_page.open_ngoisao("https://ngoisao.vnexpress.net/video")
            self.ngoisao_page.click_btn_download()
            file_name = self.ngoisao_page.wait_for_downloads(
